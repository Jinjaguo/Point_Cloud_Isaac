...
def update(q_state, contact_scenes, finger_list=('index','middle','thumb'), max_steps=200, threshold=1e-3):
    ...
    """
    q_tensor = q_state.reshape(1, 1, 16).clone().detach().to(device=q_state.device)
    ...
    while step < max_steps:
        ...
        for finger_name in finger_list:
            ...
            # calculate sdf and grad for the current finger
            g, grad_g, _ = contact_constraints(q_tensor, finger_name,
                                               contact_scenes, compute_grads=True, compute_hess=False, terminal=False,
                                               projected_diffusion=False)
            # print(grad_g)
            ...
            if finger_name == 'thumb':
                #
                gradgf_flat = -grad_g.view(-1, q_tensor.shape[-1])  # shape=[1, 16]
            else:
                gradgf_flat = grad_g.view(-1, q_tensor.shape[-1])  # shape=[1, 16]
        ...
        q_tensor.grad = total_grad / len(finger_list)
